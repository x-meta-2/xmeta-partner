  ...
  return (
    <Card className="gap-5 p-5">
      <div className="flex items-center justify-between">
        <div>
          <div className="text-sm text-muted-foreground">{t('partner:dashboard.tier.title')}</div>
          <div className="mt-1 flex items-center gap-2">
            <Trophy className="size-5 text-amber-500" />
            <span className="text-xl font-semibold">{currentTier.name}</span>
            <span className="text-sm text-muted-foreground">
              · {formatRate(currentTier.commissionRate)} {t('partner:dashboard.tier.rate')}
            </span>
          </div>
        </div>
        {nextTier && (
          <div className="text-right">
            <div className="text-xs text-muted-foreground">{t('partner:performance.nextTier')}</div>
